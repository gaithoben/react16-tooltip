...
class Tooltip extends Component {
  ...
  static get propTypes() {
    return {
      ...
      el: PropTypes.object,
      place: PropTypes.oneOfType([PropTypes.string, PropTypes.array])
        .isRequired,
      ...
      <div>
        ...
        >
          ...
          <div
            ...
            key={`a-${place}`}
          >
            <span ref="border" style={style.border} key={`b-${place}`} />
          </div>
        </div>
        <div
          ref="shadow"
...
function select(state, ownProps) {
  ...
  if (1 < names.length) {
    console.error(
      `<Tooltip> does not accept a list of names as 'name' props: ${names}`
    );
  }
  const name = names[0];
